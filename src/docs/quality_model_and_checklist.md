...
- F‑COV‑3: Сценарии «подключение/pairing», «выбор модели», «чат + streaming + cancel» покрыты тестами (unit/integration).
...
- PERF‑2: Измерены TTFT и полное время ответа (p50/p95) при реальной сети (включая VPN/4G).
...
- SEC‑6: Домашняя Ollama не доступна анонимно из интернета; доступ извне — только через VPN/туннель и аутентифицированный gateway.
