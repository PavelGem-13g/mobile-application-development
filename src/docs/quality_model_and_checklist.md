...
- UX‑6: Реализован prompt для UX‑feedback после завершения сценария (задержка 10 секунд).
...
- PERF‑5: Есть performance‑тест на загрузку данных (refresh models) и план load‑теста backend.
...
- REL‑4: Есть ключевая метрика надежности + инструмент мониторинга (Prometheus/Grafana).
- REL‑5: Crash monitoring пока не подключен (запланировано).

### 2.5. Security & privacy

- SEC‑1: Трафик между приложением и gateway идет по HTTPS/TLS (или внутри VPN) без внешнего доступа.
- SEC‑2: Токены сейчас хранятся в `AppStorage` (нужно перенести в Keychain).
...
- 0: отсутствует
- 1: кнопка feedback
- 2: prompt после завершения сценария (см. `src/docs/metrics_monitoring_feedback.md`)
